...
parser.add_argument(
    "--epochs", dest="epochs", default=20, type=int, help="Number of epochs."
)
parser.add_argument(
    "--steps", dest="steps", default=40, type=int, help="Number of steps per epoch."
)
parser.add_argument(
    "--distribute",
    dest="distribute",
...
):  # limit to 10 batches for display
...
if os.path.exists("NOE"):
    ...
    model = build_and_compile_model(
        out_steps=FORECAST_HORIZON, num_features=len(targets)
    )
    history = model.fit(
        train_dataset,
        epochs=50,
...
for x_batch, y_batch in train_dataset.take(1):
    input_seq = x_batch[0]  # First sequence in batch
    true_future = y_batch[0]  # Corresponding true future values
    prediction = model.predict(np.expand_dims(input_seq, axis=0))[
        0
    ]  # Shape: (output steps,)
    print(y_batch[0])
    print(prediction)
